...
export const useTaskStore = create<TaskStore>((set, get) => ({
  ...
  toggleTaskCompletion: (id) => {
    set((state) => {
      ...
      // Note: Recurrence functionality has been removed
      // Tasks are now one-time only
  ...
  migrateOldData: () => {
    ...
    if (oldTasks.length === 0) {
      // No old data, create default Inbox project
      ...
      const defaultProjects = [defaultInboxProject];
